...
class SiteController extends Controller
{
    ...
    {
        // require_once dirname(__FILE__).DS.'..'.DS.'extensions'.DS.'campaign_monitor'.DS.'class'.DS.'base_classes.php';
        require_once dirname(__FILE__) . DS . '..' . DS . 'extensions' . DS . 'campaign_monitor' . DS . 'csrest_subscribers.php';
        require_once dirname(__FILE__) . DS . '..' . DS . 'extensions' . DS . 'campaign_monitor' . DS . 'csrest_lists.php';

        $this->Lists = new CS_REST_Lists;
...
fb('index started');
        ...
        $lists = $this->Lists->get_active_subscribers();

        $subscribers_objects = $lists->response->Results;
        $subscribers = $this->subscribers_object_ToArray($subscribers_objects);

        // Get rawData and create dataProvider

        $filteredData = $filtersForm->filter($subscribers);
      ...
      /*  $listDataProvider = new CArrayDataProvider($subscribers, array(
            'id' => 'subscribers',
            'keyField' => 'email',
            'sort' => array(
                'attributes' => array(
                    'name', 'email', 'age', 'gender', 'date', 'state'
                ),
            ),
            'pagination' => array(
                'pageSize' => 6,
    ...
    public function actionAddSubscriber()
    {
        ...
        if (isset($_POST['SubscriberForm'])) {
            $model->attributes = $_POST['SubscriberForm'];
            if ($model->validate()) {

                $subscriber = array(
                    'EmailAddress' => $_POST['SubscriberForm']['email'],
                    'Name' => $_POST['SubscriberForm']['name'],
                    'CustomFields' => array(
                        array(
                            'Key' => 'age',
                            'Value' => $_POST['SubscriberForm']['age']
                        ),
                        array(
                            'Key' => 'gender',
                            'Value' => $_POST['SubscriberForm']['gender']
                        ),
                    )
                );
                $this->Subscribers = new CS_REST_Subscribers;
                $Subscribers = $this->Subscribers;
                $response = $Subscribers->add($subscriber);

                if ($response->was_successful()) {
                    $msg = Yii::t('subscribed', 'Subscriber\'s email <strong>' . $response->response . ' </strong>added successfully! (you may need to wait half a minute or so,and then refresh the page for the Campaign Monitor to update the database!)');
                    Yii::app()->user->setFlash('success', $msg);
                    fb($response, 'Added Success');
                    $this->redirect(bu() . '/site/index');
                } else {
                    $msg = Yii::t('subscribed', 'Error !  ' . $response->response->Message);
                    fb($response, 'Added Error');
        ...
        $this->render('add_subscriber', array('model' => $model));
    }
    ...
    {
        $this->Subscribers = new CS_REST_Subscribers;
        $email = CHtml::decode($_GET['email']);

        $response = $this->Subscribers->delete($email);


        if ($response->was_successful()) {
            fb($response, 'delete success');
            $msg = Yii::t('subscribed', 'Subscriber deleted successfully (you may need to wait half a minute or so,and then refresh the page for the Campaign Monitor to update the database!)');
            Yii::app()->user->setFlash('success', $msg);
            $this->redirect(bu() . '/site/index');
        } else {
            fb($response, 'delete error');
            $msg = Yii::t('subscribed', 'Error in deleting subscriber!');
            Yii::app()->user->setFlash('danger', $msg);
        }
...
//fb($email,'email');
    ...
    private function subscribers_object_ToArray($list)
    {
        $arr = array();
        foreach ($list as $item) {
            $arr[] = array('name' => $item->Name,
                ...
                'datetime' => $item->Date,
                'state' => $item->State,
                'age' => isset($item->CustomFields[0]->Value) ? $item->CustomFields[0]->Value : '',
                'gender' => isset($item->CustomFields[1]->Value) ? $item->CustomFields[1]->Value : '',
            );
